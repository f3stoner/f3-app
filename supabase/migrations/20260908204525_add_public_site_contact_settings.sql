-- Add contact-email routing and expose existing public contact/social settings
-- through the authenticated Public Site Settings management RPC.
--
-- Public site visitors do NOT receive contact_email through load_public_region_site.
-- That address is intended for future server-side contact-form delivery.
...
begin
    ...
    return jsonb_build_object(
        ...
        'contactUrl', v_config.contact_url,
        'joinUrl', v_config.join_url,
        'socialLinks', v_config.social_links,
        'contactEmail', v_config.contact_email,
...
begin
    ...
    v_contact_url := nullif(btrim(p_contact_url), '');
    v_join_url := nullif(btrim(p_join_url), '');
    v_contact_email := nullif(lower(btrim(p_contact_email)), '');

    v_social_links := coalesce(p_social_links, '[]'::jsonb);

    if jsonb_typeof(v_social_links) <> 'array' then
    ...
    if v_contact_url is not null
       and v_contact_url !~ '^https://[^[:space:]]+$' then
        raise exception 'Contact URL must use https://';
    end if;

    if v_join_url is not null
       and v_join_url !~ '^https://[^[:space:]]+$' then
        raise exception 'Join URL must use https://';
    end if;
