...
begin
    ...
    /*
     * A regional stats row exists only when the canonical
     * member has activity or an imported baseline in the
     * target region.
     */
    if not exists (
        select 1
    ...
    and not exists (
        ...
        where msb.region_id = target_region_id
          and msb.member_id = target_member_id
    )
    then
        return;
    end if;
    ...
    select
        ...
        count(distinct s.id) filter (
            where exists (
                ...
                where
                    fng.fng_obj ->> 'invitedById' =
                        m.id::text
                    or
                    fng.fng_obj ->> 'invited_by_id' =
                        m.id::text
            )
        ) as fngs_eh,
        ...
        case
            /*
             * members.first_post_date historically describes
             * the member's home-region history. Do not use it
             * for an external member's target-region stats.
             */
            when m.region_id = target_region_id then
                coalesce(
        ...
        (
            ...
            join public.aos a
              on a.id = s2.ao_id
            where s2.region_id = target_region_id
              and s2.ao_id is not null
              and (
                  coalesce(
                      s2.attendee_ids,
                      '[]'::jsonb
    ...
    left join public.sessions s
      on s.region_id = target_region_id
     and (
          coalesce(s.attendee_ids, '[]'::jsonb)
              ? m.id::text
