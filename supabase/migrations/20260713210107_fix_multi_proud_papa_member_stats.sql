CREATE OR REPLACE FUNCTION public.rebuild_member_stats_for_member(target_region_id uuid, target_member_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
begin
  ...
  select
    ...
    (
        select count(distinct mi.member_id)
        from public.member_inviters mi
        join public.members invited_member
            on invited_member.id = mi.member_id
        where mi.inviter_member_id = m.id
        and invited_member.region_id = m.region_id
    ) as fngs_eh,
