/*
 * Reconcile every achievement-style feed projection produced
 * by one newly created session.
 *
 * Future session achievement types should be added here rather
 * than directly to save_session_command.
 */
create or replace function public.reconcile_region_feed_achievements_for_session(
    p_session_id uuid
)
returns void
language plpgsql
...
begin
    ...
    perform public.reconcile_region_feed_vqs_for_session(
        p_session_id
    );
end;
$function$;
...
public.reconcile_region_feed_achievements_for_session(uuid)
...
begin
    ...
    * idempotent session-completed event, then reconciles all
    * session-driven achievement events.
    *
    * Future feed achievements should be registered inside
    * reconcile_region_feed_achievements_for_session() rather than
    * directly in this command.
    *
    ...
    */
    if p_mode = 'create' then
        ...
        perform public.reconcile_region_feed_achievements_for_session(
            target_session_id
        );
    end if;
