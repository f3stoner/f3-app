...
begin
    /*
     * Normalize and bound the requested date window.
     */
    ...
    /*
     * Resolve only an explicitly enabled public region.
     */
    select r.id
    into v_region_id
    ...
    select jsonb_build_object(
        ...
        jsonb_build_object(
            ...
            'timezone', c.timezone,
            'logoUrl', c.logo_asset_url,
            'heroUrl', c.hero_asset_url,
        ...
        coalesce(
            (
                ...
                /*
                 * q_slots determine whether the concrete occurrence exists.
                 *
                 * Until q_slots snapshot occurrence time/site/duration,
                 * use the applicable canonical recurring schedule as fallback
                 * metadata for that occurrence.
                 */
                left join lateral (
                    select
                        ars.site_id,
                    ...
                    from public.ao_recurring_schedules ars
                    where ars.region_id = v_region_id
                      and ars.ao_id = qs.ao_id
                      and ars.weekday = extract(isodow from qs.date)::integer
                      and ars.is_active = true
                      and (
                          ars.effective_start_date is null
                          or ars.effective_start_date <= qs.date
                      )
                      and (
                          ars.effective_end_date is null
                          or ars.effective_end_date >= qs.date
                      )
                    order by
                        ars.effective_start_date desc nulls last,
                        ars.created_at desc
                    limit 1
                ) recurring
                  on true
                ...
                /*
                 * Site precedence:
                 * 1. Concrete q_slot snapshot
                 * 2. Applicable recurring schedule
                 * 3. AO default site
                 */
