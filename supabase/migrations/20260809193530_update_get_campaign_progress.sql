/*
 * Expand campaign progress support for cumulative manual
 * quantity challenges.
 *
 * Existing behavior remains unchanged for:
 * - regional FNG campaigns;
 * - individual post-count challenges;
 * - daily manual quantity challenges.
 *
 * Cumulative manual quantity challenges now also return
 * today's contribution through todayCurrent so the same
 * incremental quantity logger can be reused safely.
 */
