CREATE OR REPLACE FUNCTION public.save_announcement_command(p_action text, p_region_id uuid, p_announcement_id uuid DEFAULT NULL::uuid, p_title text DEFAULT NULL::text, p_body text DEFAULT NULL::text, p_scope text DEFAULT NULL::text, p_ao_id uuid DEFAULT NULL::uuid, p_starts_on date DEFAULT NULL::date, p_ends_on date DEFAULT NULL::date, p_is_active boolean DEFAULT NULL::boolean, p_include_in_backblast boolean DEFAULT NULL::boolean, p_link_url text DEFAULT NULL::text, p_link_label text DEFAULT NULL::text, p_reorder_items jsonb DEFAULT NULL::jsonb, p_update_fields text[] DEFAULT NULL::text[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    IF normalized_action = 'create' THEN
        ...
        if saved_announcement.is_active then
            perform public.reconcile_region_feed_for_announcement(
                saved_announcement.id
            );
        end if;
